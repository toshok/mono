...
static void
null_ephemerons_without_vtable (void)
{
...
mono_gc_clear_domain (MonoDomain * domain)
{
	...
	null_ephemerons_without_vtable ();

	for (i = GENERATION_NURSERY; i < GENERATION_MAX; ++i)
		sgen_null_links_with_null_vtable (i);
...
mono_gc_get_managed_allocator (MonoClass *klass, gboolean for_box)
{
	...
	if (ALIGN_TO (klass->instance_size, SGEN_ALLOC_ALIGN) < SGEN_MAX_SMALL_OBJ_SIZE && !mono_class_is_open_constructed_type (&klass->byval_arg))
